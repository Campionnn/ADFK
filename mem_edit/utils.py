...
def search_buffer_between(needle_buffer: ctypes_buffer_t,
                          haystack_buffer: ctypes_buffer_t,
                          max_diff: float,
                          ) -> List[int]:
    found = []
    haystack = np.frombuffer(haystack_buffer, dtype=np.float32)
    needle = float(needle_buffer.value)
    for i in range(len(haystack)):
        if abs(haystack[i] - needle) <= max_diff:
            # print(f"Checking {i} {haystack[i]} {needle} {abs(haystack[i] - needle)}")
            found.append(i)
    return found
