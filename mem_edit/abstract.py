"""
Abstract class for cross-platform mem_edit editing.
"""
...
class Process(metaclass=ABCMeta):
    ...
    This class is used to interact with processes running on the system
      (i.e., by reading from or writing to the mem_edit used by a given process).
    ...
    Once you have found the pid, you are ready to construct an instance of Process
      and use it to read and write to mem_edit. Once you are done with the process,
    ...
    To read/write to mem_edit, first create a buffer using ctypes:
    ```
    ...
    Searching for a value can be done in a number of ways:
      ...
      Search the entire mem_edit space:
        `found_addresses = p.search_all_memory(buffer0, writeable_only=False)`

    You can also get a list of which regions in mem_edit are mapped (readable):
    ...
    def __init__(self, process_id: int):
        ...
        Constructing a Process object prepares the process with specified process_id for
          mem_edit editing. Finding the `process_id` for the process you want to edit is often
    ...
    def read_memory(self, base_address: int, read_buffer: ctypes_buffer_t) -> ctypes_buffer_t:
        ...
        Args:
            ...
            read_buffer: A `ctypes` object, for example. `ctypes.c_ulong()`,
                ...
                overwritten with the contents of the process's mem_edit starting at `base_address`.
    ...
    def list_mapped_regions(self, writeable_only=True) -> List[Tuple[int, int]]:
        ...
        Returns:
            List of `(start_address, stop_address)` for each accessible mem_edit region.
        """
        pass
    ...
    def search_addresses(self,
        ...
        Args:
            ...
            needle_buffer: The value to search for. This should be a `ctypes` object of the same
                ...
                mem_edit at each of the given addresses.
    ...
    def search_all_memory(self,
        ...
        """
        Search the entire mem_edit space accessible to the process for the provided value.

        Args:
            needle_buffer: The value to search for. This should be a ctypes object of the same
                sorts as used by `.read_memory(...)`, which will be compared to the contents of
                mem_edit at each accessible address.
            writeable_only: If `True`, only search regions where the process has write access.
                Default `True`.
            verbatim: If `True`, perform bitwise comparison when searching for `needle_buffer`.
                If `False`, perform `utils.ctypes_equal-based` comparison. Default `True`.
            max_diff: If set, use `utils.search_buffer_between` to search for `needle_buffer`
        ...
        size = len(self.list_mapped_regions(writeable_only))
        for i, region_range in enumerate(self.list_mapped_regions(writeable_only)):
            start = region_range[0]
            stop = region_range[1]
            try:
                ...
                if i % 100 == 0:
                    print(f"Progress: {(i/size)*100}%")
                if max_diff:
                    found += [offset + start for offset in utils.search_buffer_between(needle_buffer, region_buffer, max_diff)]
                else:
